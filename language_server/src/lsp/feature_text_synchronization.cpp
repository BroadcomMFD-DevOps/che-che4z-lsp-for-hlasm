...
void feature_text_synchronization::on_did_open(const nlohmann::json& params)
{
    ...
    std::string doc_uri = text_doc.at("uri").get<std::string>();
    auto version = text_doc.at("version").get<nlohmann::json::number_unsigned_t>();
    std::string text = text_doc.at("text").get<std::string>();
...
void feature_text_synchronization::on_did_change(const nlohmann::json& params)
{
    ...
    {
        texts[i] = ch.at("text").get<std::string>();
        ...
        {
            changes.emplace_back(parse_range(ch.at("range")), texts[i].c_str(), texts[i].size());
        }
...
{
    std::string doc_uri = params.at("textDocument").at("uri").get<std::string>();
