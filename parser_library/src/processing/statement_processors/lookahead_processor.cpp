...
{
    const auto& instr = [&instruction_set = hlasm_ctx.instruction_set()](const std::string& opcode) {
        if (auto mnemonic = instruction_set.find_mnemonic_codes(opcode))
            return *mnemonic->instruction();
        else
            return instruction_set.get_machine_instructions(opcode);
