    ...
    const resolved_statement& stmt, context::dependency_solver& dep_solver, diagnostic_collector add_diagnostic)
{
    ...
    const auto& mnemonic = context::instruction_sets::get_mnemonic_codes(instr_name);
    ...
    const context::mnemonic_code* mnemonic)
{
    ...
    auto tmp = context::instruction_sets::find_assembler_instructions(*stmt.opcode_ref().value);
    ...
    if (auto mach_op = dynamic_cast<const semantics::machine_operand*>(&ev_op))
    {
        ...
        const auto* instr = mnemonic ? mnemonic->instruction()
                                     : &context::instruction_sets::get_machine_instructions(*stmt.opcode_ref().value);
    ...
    const diagnosable_ctx& diagnoser)
{
    ...
    if (auto mnem_tmp = context::instruction_sets::find_mnemonic_codes(instruction_name))
    {
