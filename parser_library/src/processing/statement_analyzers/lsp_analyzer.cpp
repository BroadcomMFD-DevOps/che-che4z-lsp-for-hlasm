...
void lsp_analyzer::analyze(const semantics::preprocessor_statement_si& statement)
{
    ...
    if (const auto& operands = statement.m_details.operands.items; statement.m_copylike && !operands.empty())
        add_copy_operand(hlasm_ctx_.ids().add(operands.back().name), operands.back().r, false);
