...
bool parser_impl::NOT(const antlr4::Token* token) const
{
    ...
    static constexpr const auto upper_second = [](unsigned char l, unsigned char r) { return l == ::toupper(r); };
    return std::ranges::equal(NOT_OPERATOR, token_txt, upper_second);
}
