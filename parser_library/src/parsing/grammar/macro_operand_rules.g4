...
mac_entry_basic_tokens [concat_chain* chain]
	...
	(
		...
		{
			$chain->push_back(char_str_conc($token.text, provider.get_range($token)));
		}
		| equals		{$chain->push_back(equals_conc());}
		| dot			{$chain->push_back(dot_conc());}
		| token=(IDENTIFIER|NUM|ORDSYMBOL)
		{
			auto r = provider.get_range($token);
			$chain->push_back(char_str_conc($token.text, r));
		...
		(
			vs_id tmp=subscript
			{
				...
				$chain->push_back(var_sym_conc(std::make_unique<basic_variable_symbol>(id, std::move($tmp.value), r)));
			...
			{
				$chain->push_back(var_sym_conc(std::make_unique<created_variable_symbol>($clc.ctx ? std::move($clc.concat_list) : concat_chain{},std::move($subscript.value),provider.get_range($AMPERSAND,$subscript.ctx->getStop()))));
				collector.add_hl_symbol(token_info(provider.get_range($AMPERSAND),hl_scopes::var_symbol));
			}
			|
			AMPERSAND
			{
				$chain->push_back(char_str_conc("&&", provider.get_range(_input->LT(-2),_input->LT(-1))));
			}
		)
		|
		lpar
		...
		)?
		{
			...
			$chain->push_back(sublist_conc(std::move(sublist)));
		}
		rpar
	)
	;
...
mac_entry returns [concat_chain chain]
	...
	(
		...
		at=ATTR
		{
			$chain.push_back(char_str_conc("'", provider.get_range($at)));
		}
		(
			...
			(
				(
					equals
					{
						$chain.push_back(equals_conc());
					}
				)?
				ORDSYMBOL
				{
					auto r = provider.get_range($ORDSYMBOL);
					$chain.push_back(char_str_conc($ORDSYMBOL.text, r));
					collector.add_hl_symbol(token_info(r, hl_scopes::operand));
				}
				|
				AMPERSAND
				(
					...
					(
						ap=(APOSTROPHE|ATTR)
						{
							$chain.push_back(char_str_conc("'", provider.get_range($ap)));
