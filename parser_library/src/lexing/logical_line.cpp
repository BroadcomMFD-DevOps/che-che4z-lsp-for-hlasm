...
bool append_to_logical_line(logical_line& out, std::string_view& input, const logical_line_extractor_args& opts)
{
    ...
    auto [code_start, utf16_begin_offset] = utils::skip_chars(line, opts.begin - 1);
    auto after_code = utils::skip_chars(code_start, opts.end + 1 - opts.begin).first;
    ...
    // line is continued
    segment.ignore = utils::skip_chars(after_code, 1).first;
...
void finish_logical_line(logical_line& out, const logical_line_extractor_args& opts)
{
    ...
    for (size_t i = 1; i < out.segments.size(); ++i)
    {
        ...
        auto [code_without_cont, utf16_skipped] = utils::skip_chars(s.code, cont_size);
