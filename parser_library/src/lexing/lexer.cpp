...
void lexer::create_token(int ttype, unsigned channel)
{
    ...
    if (input_state_.next == token_start_state_.next && ttype != antlr4::Token::EOF)
        return;
...
bool lexer::eof() const { return *input_state_.next == static_cast<char_t>(-1); }
...
{
    start_token();
    while (*input_state_.next != '\n' && !eof() && *input_state_.next != (char_t)-1)
        consume();
...
bool lexer::is_process() const
{
    if (!process_allowed_)
        return false;
    for (size_t p = 0; unsigned char c : std::string_view("*PROCESS"))
    {
        char_t next = input_state_.next[p];
        if (next > (unsigned char)-1 || next != (unsigned char)toupper(next))
            return false;
    }
    return true;
}
