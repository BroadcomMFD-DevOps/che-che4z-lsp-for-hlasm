...
void transform_reloc_imm_operands(semantics::operand_list& op_list, context::id_index instruction)
{
    ...
    if (auto mnem_tmp = context::instruction_sets::find_mnemonic_codes(*instruction))
        mask = mnem_tmp->reladdr_mask().mask();
    else
        mask = context::instruction_sets::get_machine_instructions(*instruction).reladdr_mask().mask();
