...
{
    return address({ &owner, id_storage::empty_id }, curr_data().storage, curr_data().spaces());
}
...
address location_counter::current_address_for_alignment_evaluation(alignment align) const
{
    ...
    return address({ &owner, id_storage::empty_id }, offset, alignment_spaces);
}
...
aligned_addr location_counter::reserve_storage_area(size_t length, alignment a)
{
    ...
    return std::make_pair(
        address({ &owner, id_storage::empty_id }, (int)curr_data().storage, curr_data().spaces()), sp);
}
...
std::pair<space_ptr, std::vector<address>> location_counter::set_available_value()
{
    ...
    for (auto& entry : org_data_)
        addr_arr.emplace_back(address::base { &owner, id_storage::empty_id }, entry.storage, entry.spaces());
