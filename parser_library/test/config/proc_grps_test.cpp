...
TEST(proc_grps, assembler_options_read)
{
    const auto cases = {
        ...
        std::make_pair(R"({"SYSTEM_ARCH":"ZS9"})"_json, assembler_options { "", "", "ZS9" }),
        std::make_pair(R"({"SYSTEM_ID":"VSE"})"_json, assembler_options { "", "", "", "VSE" }),
        std::make_pair(R"({"PROFILE":"MAC","SYSPARM":"TESTPARM","SYSTEM_ARCH":"ZS9","SYSTEM_ID":"VSE"})"_json,
    ...
    for (const auto& [input, expected] : cases)
    {
        ...
        EXPECT_EQ(ao.system_architecture, expected.system_architecture);
